...
function genNewUser(context, events, done) {
	...
	context.vars.userid = first + "." + last + "." + `${faker.number.int(1000000)}`
...
function genNewUserReply(requestParams, response, context, ee, next) {
	...
	if( response.statusCode >= 200 && response.statusCode < 300 && response.body.length > 0)  {
		...
		console.log("usernick: " + context.vars.userid)
		console.log("password: " + context.vars.pwd)

		//users array has actual user id
		users.push(id);
		fs.writeFileSync('users.data', JSON.stringify(users));
	}
    return next()
...
function genNewUserReply2(requestParams, response, context, ee, next) {
	/*if( response.statusCode >= 200 && response.statusCode < 300 && response.body.length > 0)  {
		//let u = JSON.parse( response.body)
		context.vars.usernick = context.vars.userid;
		context.vars.actualuserid = response.body.split(" - ", 7)[1];
    ...
    return next()*/
	console.log("we out user reply 2")
	if( response.statusCode >= 200 && response.statusCode < 300 && response.body.length > 0)  {
		//let u = JSON.parse( response.body)
		console.log("we in user reply 2")
		//context.vars.userid is the nickname
...
function genNewHouseReply(requestParams, response, context, ee, next) {
	...
	if( response.statusCode >= 200 && response.statusCode < 300 && response.body.length > 0)  {
		...
		let houses = userHouses.get(id);

		if (houses !== undefined){
			houses.push(id)
			userHouses.set(context.vars.userid, houses)
		} else {
			let hArr = [];
			hArr.push(id);
			userHouses.set(context.vars.userid, hArr)
		}
...
function selectUserSkewed(context, events, done) {
	if( users.length > 0) {
		...
		let iduser = users.sampleSkewed()
		context.vars.userid = iduser;
		context.vars.loggedName = usersIdToNick.get(context.vars.userid)
		context.vars.loggedPassword = nickPass.get(context.vars.loggedName)
...
function getRandomUser(context, events, done){
	context.vars.randomUserId = users.sampleSkewed()
	return done()
}
